...
\begin{frame}{What about non-linear Data Sets?}{}
	...
	\end{itemize}
		...
		\begin{figure}
			\centering
			\includegraphics[scale=0.33]{10_deep_learning/02_img/rbfn}
		\end{figure}
	}
\end{frame}
...
\begin{frame}{Multi-Layer Perceptrons}{}
	...
	\divideTwo{0.49}{
		\begin{align*}
			\bm{z}^{[1]}(\bm{(x)}) &= g^{[1]}(\bm{\Theta}^{[1]} \bm{x})						\\
			\dots 													\\
			\bm{z}^{[\lambda]}(\bm{x}) &= g^{[\lambda]}(\bm{\Theta}^{[\lambda]} \bm{z}^{[\lambda - 1]}(\bm{x}))
		\end{align*}
	}{0.49}{
		\textbf{Prediction:}
		\begin{equation*}
			y_{p} = \argmax_k \bm{z}^{[\lambda]}(\bm{x})
		\end{equation*}
	}
\end{frame}
...
\begin{frame}{ReLU (Ctd.)}{}\important
	\begin{itemize}
		...
		\item \Highlight{But:} Pay attention to the initialization of your parameters to avoid \textit{`dying ReLUs'} (parameter settings where single neurons will always output 0)
...
\begin{frame}{Convolutional Neural Networks (CNNs)}{}
	\begin{itemize}
		...
		\item \textbf{Solution:} Use \highlight{convolution}!
	\end{itemize}
\end{frame}
