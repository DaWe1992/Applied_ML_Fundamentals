...
\begin{frame}{Softmax Activation}{}
	\begin{boxBlueNoFrame}
		\begin{equation*}
			softmax_i(\bm{x}) = \frac{\exp\{ x_i\}}{\sum_{k=1}^{\kappa} \exp\{x_k\}}
		\end{equation*}
	\end{boxBlueNoFrame}
	\begin{itemize}
		\item Softmax is a \textbf{global activation function} (i.\,e. it depends on the preactivations from the other units in the layer)
		\item It is used to squash the last layer's activations into a probability distribution, such that the activations sum to 1
	\end{itemize}
\end{frame}
