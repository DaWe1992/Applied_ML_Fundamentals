...
\begin{frame}{Value Iteration Algorithm}{}
	\begin{algorithm}[H]
		...
		Output a deterministic policy $\pi \approx \pi^*$, such that:
 		...
 		\caption{Value Iteration (deterministic case)}
	\end{algorithm}
\end{frame}
...
\begin{frame}[plain]{}{}
	\begin{algorithm}[H]
		...
		\DontPrintSemicolon
 		...
 		\While(\tcp*[h]{Step \ding{182}: Policy evaluation}){$\Delta > \varepsilon$}{
 			$\Delta \longleftarrow 0$\;
 			\ForEach{$s \in \mathcal{S}$}{
 				$v \longleftarrow V(s)$\;
				$V(s) \longleftarrow r + \gamma V(s') $\;
		...
		policy\_stable $\longleftarrow$ true \tcp*[h]{Step \ding{183}: Policy improvement}\;
		\ForEach{$s \in \mathcal{S}$}{
			old\_action $\longleftarrow \pi(s)$\;
			$\pi(s) \longleftarrow \argmax_{a}[r + \gamma V(s')]$\;
			\If{old\_action $\ne \pi(s)$}{
				policy\_stable $\longleftarrow$ false\;
			}
		}
		\If{policy\_stable}{
			\textbf{return} $V \approx v^*$ and $\pi \approx \pi^*$, go to line 2 otherwise
		}
...
\begin{frame}{Policy Iteration Example: Grid World}{}
	\begin{figure}
		\centering
		\includegraphics[scale=0.275]{14_rl/02_img/optimal_policy_pi}
	\end{figure}
\end{frame}
...
\begin{frame}{SARSA: State-Action-Reward-State-Action}{}
	\begin{itemize}
		...
		\item Stands for `\highlight{S}tate-\highlight{A}ction-\highlight{R}eward-\highlight{S}tate-\highlight{A}ction'
	...
	\begin{align*}
		\nabla\mathbb{E}_{\pi_{\bm{\theta}}}
			&= \nabla\int \pi(\tau) r(\tau) \diff\tau
				...
				\Leftrightarrow \int \pi(\tau) \nabla\log\pi(\tau) r(\tau) \diff\tau \\
			&\Leftrightarrow \mathbb{E}_{\pi_{\bm{\theta}}} [r(\tau) \nabla\log\pi(\tau)] \\
		\pi_{\bm{\theta}}(\tau)
			&= p(s_0) \cdot \prod_{t=1}^T \pi_{\bm{\theta}}(a_t \vert s_t) \cdot p(s_{t+1}, r_{t+1} \vert s_t, a_t)
	\end{align*}
