...
\begin{frame}{MENACE: Setup}{}
	\begin{itemize}
		...
		\end{itemize}
		\item \highlight{How can good moves be positively reinforced?}
	\end{itemize}
\end{frame}
...
\begin{frame}{Reinforcement Learning in MENACE}{}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Game \highlight{won}: Marble of corresponding color is added (positive reinforcement)
...
\begin{frame}{Algorithm for $Q$-Learning (Ctd.)}{}
	\divideTwo{0.60}{
		\begin{itemize}
			...
			\item The $Q$-function is represented by a table
			\begin{itemize}
				\item One entry per state-action pair
				\item The entry for pair $\langle s, a \rangle$ stores $\widehat{Q}(s, a)$
				\item Initially, table is filled with random numbers or zeroes
			\end{itemize}
			\item \textbf{Problem}: The table may become large if there are many states and actions
		\end{itemize}
	}{0.39}{
		\vspace*{2mm}
...
\begin{frame}{SARSA: State-Action-Reward-State-Action}{}
	\begin{itemize}
		...
		\item Instead it uses the current policy ($\Rightarrow$ \highlight{on-policy update})
		\begin{equation}
...
\begin{frame}{Non-Deterministic Rewards and Actions (Ctd.)}{}
	...
	\begin{boxBlueNoFrame}
		E.\,g. TD-Gammon trained for \highlight{\underline{1.5 million (!)}} Backgammon games.
	\end{boxBlueNoFrame}
\end{frame}
...
\begin{frame}{Final Example (Ctd.)}{}
	\highlight{a)} Specify the reward function $r(s, a)$ for all state-action pairs.
...
% Final Example (Ctd.)
\begin{frame}{Final Example (Ctd.)}{}
	\highlight{b)} Calculate the value function $V^{\pi}(s)$ for all states $s$ given policy $\pi$:
...
% Final Example (Ctd.)
\begin{frame}{Final Example (Ctd.)}{}
	\highlight{c)} How would \textbf{Policy Improvement} change the policy $\pi$ from task b) for \textbf{e}? \\[5mm]
...
% Final Example (Ctd.)
\begin{frame}{Final Example (Ctd.)}{}
	\highlight{d)} Think of an optimal way to reach the goal state for each $s$. Calculate $V^{\pi^*}(s)$. \\[5mm]
...
% Final Example (Ctd.)
\begin{frame}{Final Example (Ctd.)}{}
	\highlight{e)} Calculate the optimal $Q$-values for all possible state-action pairs. \\[5mm]
...
\begin{frame}{Summary}{}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \highlight{Credit assignment problem}: Which move(s) is / are responsible for success / failure?
			\item Example: MENACE
		\end{itemize}
...
\begin{frame}[allowframebreaks]{Recommended Literature and further Reading}{}
	...
	\begin{thebibliography}{2}
		\literature{book}{Mitchell.1997}{[1] Machine Learning}{Tom Mitchell. McGraw-Hill Science. 1997.}
			{$\rightarrow$ \href{
		...
		\literature{online}{Valkov.2017}{[2] Solving an MDP with $Q$-Learning from scratch}{Venelin Valkov. 2017.}
			{$\rightarrow$
