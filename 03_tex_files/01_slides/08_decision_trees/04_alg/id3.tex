\begin{algorithm}[H]
	...
	\If{all tuples in $\mathcal{D}$ have class $c$}{
		\Return{$N$ as leaf node labeled with class $c$} \tcp*[h]{subset is pure}
	}
	\If{$\vert Attr\_List \vert = 0$}{
		\Return{$N$ as leaf node labeled with majority class in $\mathcal{D}$} \tcp*[h]{no attribute left}
	}
	Find best split attribute $\texttt{A}^*$ and label node $N$ with $\texttt{A}^*$\;
	$Attr\_List \longleftarrow Attr\_List \backslash \{ \texttt{A}^* \}$ \tcp*[h]{remove attribute from list}\;
	\ForAll{$v \in \text{dom}(\texttt{A}^*)$}{
		...
		\Else{
			Attach node returned by $ID3(\mathcal{D}_{\texttt{A}^*=v}, Attr\_List)$ \tcp*[h]{recursion}\;
		}
	}
