...
%\newcommand{\code}[1]{\colorbox{lightgray!50}{\texttt{#1}}}
...
\begin{frame}
	...
	\vspace*{2mm}
	\begin{itemize}
		\item \textbf{Addition:} \code{a + b = np.array([5, 4, 2])}
		\item \textbf{Subtraction:} \code{a - b = np.array([-3, 2, 2])}
		\item \textbf{Multiplication:} \code{a * b = np.array([4, 3, 0])} \textit{(Hadamard/element-wise product)}
		\item \textbf{Division:} \code{a / b = np.array([0.25, 3, inf])}
		\item \textbf{Inner product:} \code{a @ b = 7} or \code{a.dot(b) = 7} \textit{(scalar product)}
		\item \textbf{Transpose:} \code{A.T} \textit{(swaps rows and columns)}
		\item \textbf{Matrix inverse:} \code{np.linalg.inv(A)}
	\end{itemize}
\end{frame}
