...
\begin{frame}{Discriminant Functions}{}
	\begin{itemize}
		...
		\item The \highlight{decision boundary} is defined by the relation: $y(\bm{x}) = 0$
...
\begin{frame}{Power of Kernels}{}
	\begin{itemize}
		...
		\begin{boxBlueNoFrame}
			\footnotesize
			\highlight{We need $\mathcal{O}(n^2)$ to compute $\varphi(\bm{x})$ and $\varphi(\bm{x'})$ but only $\mathcal{O}(n)$
