...
\begin{frame}{Bayesian Decision Theory: Bayes' Optimal Classifier (Ctd.)}{}
	\divideTwo{0.45}{
		\begin{figure}
			\centering
...
\begin{frame}{How to get the most probable Class?}{}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Finite set of classes $\mathcal{C} = \{ \mathcal{C}_1, \mathcal{C}_2, \dots, \mathcal{C}_l \}$
		...
		\begin{align}
			\mathcal{C}_{MAP}
				&= \argmax_{\mathcal{C}_k \in \mathcal{C}} \widehat{p}(\mathcal{C}_k \vert \bm{x}) \\
				&= \argmax_{\mathcal{C}_k \in \mathcal{C}} \widehat{p}(\mathcal{C}_k)
					\prod_{j=1}^m \widehat{p}(x_j \vert \mathcal{C}_k)
		\end{align}
	\end{itemize}
\end{frame}
...
% Na\"{i}ve Bayes Classifier: How to estimate the Probabilities?
\begin{frame}{Na\"{i}ve Bayes Classifier: How to estimate the Probabilities?}{}
	\begin{itemize}
		...
		\begin{align}
			...
			\widehat{p}(x_j = v \vert \mathcal{C}_k)
				&= \frac{\sum_{i=1}^n \mathbb{1}\{ x_j^{(i)} = v, y^{(i)} = \mathcal{C}_k \}}
